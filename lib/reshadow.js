...
exports.default = reshadow;
function reshadow(time, parentState, prevImpl, parentImpl) {
	var state = prevImpl.isRoot() ? parentState : parentState[prevImpl.name];
	var prevProperty = prevImpl.property && prevImpl.property;
	...
	if (prevImpl.isValid()) {
		...
		var impl = prevImpl.createCopy(time, state, parentImpl);
	...
	var shader = prevImpl.shader(state);

	// make an all new shadow model
	prevImpl.obsoleteTree();

	if (prevImpl.isRoot()) {
		var rootProperty = prevImpl.property;
